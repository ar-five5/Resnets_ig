"""
compare_heads.py - prediction head ablation study for CIFAR-10H.
"""
...
def build_model_with_head(head_name: str) -> nn.Module:
    """
    Build a ResNet-18 backbone with a specified prediction head.

    Args:
        head_name: "Linear", "MLP-2", or "MLP-3".

    Returns:
        A torch.nn.Module with the requested head.
    """
    ...
    elif head_name == "MLP-2":
        # Already matches baseline head definition.
        model.fc = nn.Sequential(
            nn.Linear(in_features, 256),
            nn.ReLU(inplace=True),
            nn.Linear(256, 10),
        )
    elif head_name == "MLP-3":
        model.fc = nn.Sequential(
            nn.Linear(in_features, 256),
            nn.ReLU(inplace=True),
            nn.Linear(256, 128),
...
def finetune_phase(
    ...
    for epoch in range(1, epochs + 1):
        ...
        train_bar = tqdm(train_loader, desc=f"Phase 2 (KL) | Epoch {epoch}/{epochs}", leave=False)
        for images, soft_labels in train_bar:
            ...
            optimizer.zero_grad()
            logits = model(images)
            loss = loss_kl(logits, soft_labels)
            loss.backward()
            optimizer.step()
        ...
        print(
            f"[Phase 2 - KL] Epoch {epoch:>3}/{epochs} | "
        ...
        if val_loss < best_val_loss:
            ...
            no_improve += 1
            if no_improve >= patience:
                print(f"[Phase 2 - KL] Early stopping at epoch {epoch}.")
                break

    return {"best_val_loss": best_val_loss}


def evaluate_model(model: nn.Module, test_loader, device: torch.device) -> Tuple[float, float, float, float]:
    """
...
def train_and_evaluate(
    ...
    """
    Train a model with the specified head and evaluate on the test split.

    Args:
        head_name: Label for the prediction head.
    ...
    slug = head_name.lower().replace("-", "").replace(" ", "_")
    config["checkpoint_path"] = str(output_dir / f"best_model_{slug}.pt")
    config["log_path"] = str(output_dir / f"training_log_{slug}.csv")

    phase1_ckpt_path = str(output_dir / f"best_model_{slug}_phase1.pt")

    init_log_file(config["log_path"])

    device = resolve_device(config)
    model = build_model_with_head(head_name).to(device)
...
def save_comparison(output_path: Path, rows: Dict[str, Tuple[float, float, float, float]]) -> None:
    """
    Save the head comparison table to a text file.

    Args:
        output_path: Output text file path.
        rows: Mapping of head name to metric tuple.
    """
    output_path.parent.mkdir(parents=True, exist_ok=True)

    with output_path.open("w", encoding="utf-8") as file:
        file.write("Prediction Head Comparison\n")
        file.write("----------------------------------\n")
        file.write("Head        KL     JSD    Cosine    Acc\n")
        file.write("----------------------------------\n")
        for head_name, (kl, jsd, cosine, acc) in rows.items():
            file.write(
                f"{head_name:<10}"
                f"{kl:>6.3f} "
                f"{jsd:>6.3f} "
...
def print_comparison(rows: Dict[str, Tuple[float, float, float, float]]) -> None:
    """Print the head comparison table to stdout."""
    print("Prediction Head Comparison")
    print("----------------------------------")
    print("Head        KL     JSD    Cosine    Acc")
    print("----------------------------------")
    for head_name, (kl, jsd, cosine, acc) in rows.items():
        print(f"{head_name:<10}{kl:>6.3f} {jsd:>6.3f} {cosine:>8.3f} {acc * 100.0:>7.2f}")
    print("----------------------------------")


def main() -> None:
    """Train and evaluate Linear, MLP-2, and MLP-3 heads."""
    ...
    output_dir = project_root / "artifacts" / "head_compare"
    output_dir.mkdir(parents=True, exist_ok=True)

    heads = ["Linear", "MLP-2", "MLP-3"]

    results: Dict[str, Tuple[float, float, float, float]] = {}
    for head_name in heads:
        print("=" * 70)
        print(f"Training and evaluating: {head_name}")
        print("=" * 70)
        metrics = train_and_evaluate(head_name, base_config, output_dir)
        results[head_name] = metrics

    print_comparison(results)

    output_path = project_root / "artifacts" / "head_comparison.txt"
