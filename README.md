...
All metrics on the CIFAR-10H test split (2,000 images). Lower KL/JSD is better; higher Cosine/Acc is better. The main model is Phase-2 fine-tuned ResNet-18 with the MLP-2 head and KL loss; ablation rows are independent re-training runs of the same backbone with one component swapped. Robustness rows apply input corruptions to the main model at inference time.

| Configuration                        | KL     | JSD    | Cosine | Top-1 Acc |
|--------------------------------------|--------|--------|--------|-----------|
| **Main model (MLP-2 head, KL loss)** | 0.2288 | 0.0498 | 0.9423 | 92.60%    |
| Head ablation — Linear               | 0.312  | 0.065  | 0.919  | 89.20%    |
| Head ablation — MLP-2                | 0.322  | 0.073  | 0.909  | 88.65%    |
| Head ablation — MLP-3                | 0.333  | 0.071  | 0.911  | 89.20%    |
| Loss ablation — KL                   | 0.320  | 0.071  | 0.912  | 88.70%    |
| Loss ablation — JSD                  | 0.426  | 0.063  | 0.917  | 89.85%    |
| Loss ablation — Soft CE              | 0.329  | 0.070  | 0.908  | 88.35%    |
| Robustness — Clean                   | 0.229  | —      | —      | 92.60%    |
| Robustness — Gaussian noise          | 2.384  | —      | —      | 30.20%    |
| Robustness — Gaussian blur           | 2.008  | —      | —      | 38.95%    |
| Robustness — Brightness shift        | 0.247  | —      | —      | 91.50%    |

Best Phase-2 validation KL: **0.2135** (`results_lock_manifest.json`). Bootstrap mean ± std for the main-model row, plus per-section breakdowns (entropy correlations, Precision@K, ablation context), are in the detailed results sections below.
